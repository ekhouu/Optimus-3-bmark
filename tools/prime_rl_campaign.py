...
def _required_count_for_item(task: str, singular: str, plural: str | None, default: int) -> int:
    text = (task or "").lower()
    target_words = [re.escape(singular)]
    if plural:
        target_words.append(re.escape(plural))
    target_expr = "|".join(target_words)
    match = re.search(rf"(\d+)\s+(?:{target_expr})\b", text)
    if match:
        try:
            return max(1, int(match.group(1)))
        except ValueError:
...
def _detect_rubric_profile(task: str, rubric_profile: str) -> str:
    if rubric_profile != "auto":
        return rubric_profile
    lower = (task or "").lower()
...
def _sum_inventory_like(final_inventory_counts: dict[str, Any], needles: list[str]) -> float:
    total = 0.0
    lowered_needles = [n.lower() for n in needles]
    for k, v in final_inventory_counts.items():
        key = str(k).lower()
        try:
            count = float(v)
        except Exception:
            continue
        if any(needle in key for needle in lowered_needles):
            total += count
    return total
...
def compute_verifier_reward(
    ...
    rubric_profile: str = "auto",
) -> dict[str, float]:
    resolved_profile = _detect_rubric_profile(task, rubric_profile)
    ...
    m_logs = 1.0 if _sum_inventory_like(final_inventory_counts, ["log"]) > 0 or any("log" in x for x in completed_goal_items) else 0.0
    m_planks = (
        1.0
        if _sum_inventory_like(final_inventory_counts, ["plank"]) > 0 or any("plank" in x for x in completed_goal_items)
        else 0.0
    )
    m_sticks = (
        1.0 if _sum_inventory_like(final_inventory_counts, ["stick"]) > 0 or any("stick" in x for x in completed_goal_items) else 0.0
    )
    ...
    m_goal_progress = (
        1.0
        if success > 0
        else (1.0 if (final_plan_length > 0 and (final_sub_task_index / final_plan_length) >= 0.80) else 0.0)
    )
    m_diamonds = 1.0 if (resolved_profile == "diamond" and (diamond_count >= required_diamonds or _have_item("diamond") > 0)) else 0.0
    cobblestone_count = _sum_inventory_like(final_inventory_counts, ["cobblestone"])
    stone_goal_target = float(_required_count_for_item(task, "cobblestone", "cobblestones", 8))
    m_cobblestone = 1.0 if cobblestone_count > 0 else 0.0
    m_stone_goal = 1.0 if cobblestone_count >= stone_goal_target else 0.0
    ...
    if resolved_profile == "stone":
        ...
        target_item = "diamond"
        required_target_count = required_diamonds
        final_target_count = diamond_count
    ...
    components = {
        "rubric_profile": resolved_profile,
        ...
        "b_furnace_bonus": b_furnace_bonus,
...
def _plot_campaign(rows: list[dict[str, Any]], out_png: Path) -> bool:
    ...
    profile = str(rows[0].get("rubric_profile", "mixed"))
    fig.suptitle(f"Prime-Style Campaign ({profile})")
    fig.tight_layout()
...
def run_campaign(cfg: CampaignConfig) -> int:
    ...
    print(
        f"[campaign] episodes={cfg.episodes} tasks={len(cfg.tasks)} task_mode={cfg.task_mode} "
        f"rubric_profile={cfg.rubric_profile}"
    )
    notifier.send(
    ...
    with episodes_csv.open("w", newline="", encoding="utf-8") as episodes_f, prime_csv.open(
        ...
        for episode_idx in range(1, cfg.episodes + 1):
            ...
            reward = compute_verifier_reward(
                summary,
                task=task,
                bonus_enabled=cfg.rubric_bonus_enabled,
                rubric_profile=cfg.rubric_profile,
            )
            row = {
                "episode": episode_idx,
                "task": task,
